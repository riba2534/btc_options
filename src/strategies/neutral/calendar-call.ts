...
const calendarCall: Strategy = {
  ...
  plainSummary: '买一张到期晚的看涨、同时卖一张同价位但到期早的看涨。赌币价就赖在这个价位附近不大动——让到期早那张先“烂掉”，你白赚它的钱，手里还留着到期晚的那张继续等机会。',
  analogy: {
    emoji: '⏳',
    title: '卖临期、留长保',
    text: '想象你手里有两张一模一样的优惠券，一张这周到期、一张下个月到期。这周到期那张时间一过就废、掉价飞快，你赶紧把它卖掉收钱；保质期长那张掉价慢，自己留着。只要东西的价钱不大涨大跌，快到期那张作废你白赚，长期那张还在手——赚的就是两张“过期速度”的差。'
  },
  pitfalls: [
    '把它当成赌方向：日历其实赌的是“不动”。币价大涨大跌反而两边都吃亏，原地踏步在行权价附近才最甜。',
    'IV 高位入场：日历是做多波动率（Vega 为正），最怕事件落地后 IV 崩塌（Vega Crush）直接砸低远月那张——利好兑现了仓位反而亏。该在 IV 偏低、预期会升时建仓。',
    '近月到期不管理：近月快到期时若价格已跑偏，要及时平仓或把近月滚到下一期；放着不管会变成只剩远月单腿的方向裸头寸。'
  ],
  quickJudge: {
    use: '预计横盘、IV 低位待升',
    avoid: '预计单边大行情、IV 高位'
  ...
  cryptoNote: 'Deribit 为欧式现金结算：日历的近月先到期、结算成现金后，剩下的远月就成了单腿裸多头，记得同步管理方向风险。成本上两腿费用叠加（期权费≈标的 0.03%/张、封顶权利金 12.5%）+近月每期滚动来回成交，容易吃掉本就微薄的净价差——务必用组合单/RFQ 按净价一次成交，别逐腿手挂（快市易留裸腿）。'
};
