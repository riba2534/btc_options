...
const shortStrangle: Strategy = {
  ...
  plainSummary: '赌 BTC 在一个较宽的区间里来回晃、既不大涨也不大跌，这样你卖出的两份期权都作废、白收两笔钱；但万一价格冲出区间——尤其大涨——亏损可能没有上限。',
  analogy: {
    emoji: '🏦',
    title: '当起不买再保险的保险公司',
    text: '你同时卖两份保险——一份赌"不会大涨"、一份赌"不会大跌"，平时稳收两笔保费。只要没出大事就白赚。但你没给自己买再保险：真碰上黑天鹅（暴涨暴跌），赔付可能把保费连本带利全吞掉，向上更是没有封顶。'
  },
  pitfalls: [
    '被"高胜率、区间又宽"迷惑——大多数时候确实稳收两笔小钱，但一次暴涨可能把之前赚的全亏回去还倒贴，且上方亏损没有上限。',
    '仓位开太大、不留缓冲——净卖方占用保证金，IV 一飙升或价格逼近行权价，保证金要求骤增，可能在最差的位置被增量强平。',
    '价格逼近某一侧还死扛——临期 Gamma 急剧放大，被测试一侧亏损加速；应及时向外滚动或减仓，而不是干等它自己回到区间内。'
  ],
  quickJudge: {
    use: '笃定宽幅震荡、IV 高位将回落',
    avoid: '可能单边突破、有重大事件或扛不住尾部风险'
  },
  greeks: {
    delta: '≈0',
    gamma: '−',
  ...
  cryptoNote: '作为净卖方在 Deribit 要占用保证金、到期前就可能被增量强平：即使现货没动，IV 单独飙升也会推高两腿空头的 mark price 触发追保；币本位（coin-margined）合约暴跌时更是"浮亏 + 抵押品 BTC 缩水"双杀。务必把仓位开小、留足保证金缓冲，别用满杠杆。'
};
