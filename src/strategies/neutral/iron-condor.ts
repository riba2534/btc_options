...
const ironCondor: Strategy = {
  ...
  plainSummary: '赌 BTC 在一个区间里晃来晃去、不大涨也不大跌。只要它待在区间内，你就稳稳收一笔钱；万一冲出区间，亏损也被提前封死、不会变成无底洞。',
  analogy: {
    emoji: '🛣️',
    title: '收过路费',
    text: '你在一段路的两头各设一个收费站收过路费（卖出期权的权利金），只要车流（币价）在中间这段路上晃悠，钱就稳稳进口袋；只有车冲出两头的收费站才会亏，但你提前在更远处装了护栏（买入的保护腿），所以亏损有上限、不会失控。',
  },
  pitfalls: [
    '以为"胜率高=稳赚"：区间内确实常赢小钱，但一次单边突破的亏损（这里最高 $3k）能吞掉好几次收益，盈亏并不对称。',
    '区间设太窄想多收权利金：价格稍微一动就被测试边界，反而更容易亏；宽区间胜率高但收得少，要权衡。',
    '临近到期还死扛：最后 1–2 周 Gamma 急剧放大，价格贴边时盈亏曲线变陡，该提前止盈或滚动，别等到期搏一把。',
  ],
  quickJudge: {
    use: '宽幅震荡、IV 高位预期回落',
    avoid: '趋势行情或事件突破前',
  ...
  cryptoNote: 'Deribit 四条腿各自付费（约标的 0.03%/张，行权再加交割费），来回开平就是约 8 次成交，会明显侵蚀这点净收入。务必用组合单/RFQ 一次性按净价成交，别手动逐腿挂单——快市里逐腿很容易只成交一两条腿、留下裸露的卖出腿，瞬间敞口失控。',
};
