...
const longStrangle: Strategy = {
  ...
  detailedAnalysis: {
    ...
    pros: [
      ...
      '方向中性:无需判断涨跌方向,只押注波动率放大即可两侧获利。',
      '成本更低:相比同到期 ATM Straddle(约 $10k),宽跨式总权利金仅 $4k,资金占用与最大亏损更小。'
    ],
    cons: [
  ...
  plainSummary: '花两笔小钱，同时押 BTC 要么大涨要么大跌——只要它真的折腾起来，不管往哪边走都能赚；要是一直不动，这两笔钱就白花了。',
  analogy: {
    emoji: '🎲',
    title: '买“两头的彩票”，但门槛更高',
    text: '像赌一场比赛会爆冷却不赌哪队赢——你买的是更便宜的“大冷门”票，所以比分差距必须拉得够大才算赢；要是平淡收场，上下两张票的钱全打水漂。',
  },
  pitfalls: [
    '以为“只要 BTC 动一下就赚”——其实波动幅度得大到盖过两笔权利金、突破上下盈亏平衡点，温和波动照样亏。',
    '在 IV（市场预期波动）高位时买入，事件一过 IV 暴跌，哪怕方向猜对，期权也可能不涨反跌（Vega crush）。',
    '为图便宜把行权价选得太远（太虚值），结果要 BTC 涨跌离谱才回本，最后多半两腿一起归零。',
  ],
  quickJudge: {
    use: '预期事件引爆暴动、且 IV 在低位',
    avoid: '横盘震荡或 IV 高位、临近到期',
  },
  greeks: {
    delta: '≈0',
    gamma: '+',
