...
const longGuts: Strategy = {
  ...
  plainSummary: '赌 BTC 接下来必有大行情、要么暴涨要么暴跌，但你不赌往哪个方向——只要动得够猛，哪边都能赚；只有横盘不动才会亏，而且因为买的是实值期权，亏损被锁在小头。',
  analogy: {
    emoji: '🎲',
    title: '押一场大行情，押金还能退大半',
    text: '你笃定 BTC 要出大事、要么暴涨要么暴跌（但不知哪边），于是两边都押。和普通跨式不同的是，你押进去的钱里有一大块是「可退押金」（实值期权的内在价值）——就算最后横盘没动，这笔押金也能拿回大半，只亏掉那点「手续费」（时间价值）；真出了大行情，照样大赚。'
  },
  pitfalls: [
    '误以为「买实值更稳更划算」——其实只是把 $10k 内在价值提前垫进去、占用大量资金，普通 Straddle/Strangle 更便宜、流动性也更好，新手基本没理由舍近求远选 Guts。',
    '以为没行情顶多亏 $6k 就躺着不管——那 $6k 全是时间价值，横盘拖得越久越接近亏满，该止损就得止损。',
    '实值期权点差大、成交清淡——建仓和平仓都容易被买卖价差和滑点吃掉一截，临到期想退出未必有好价格。'
  ],
  quickJudge: {
    use: '笃定要出大行情、不赌涨跌',
    avoid: '横盘、IV 高位、嫌占资金'
